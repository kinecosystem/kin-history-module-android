...
//      </attr>
//  </declare-styleable>
internal class KinEcosystemTabs @JvmOverloads constructor(context: Context,
    ...
    init {
        ...
        try {
            attributes?.let {
                ...
                leftColor = it.getColor(R.styleable.KinEcosystemTabsView_leftColor, ContextCompat.getColor(context,
                                                                                                           R.color.kinecosystem_purple))
                rightColor = it.getColor(R.styleable.KinEcosystemTabsView_rightColor, ContextCompat.getColor(context,
                                                                                                             R.color.kinecosystem_green))
                defaultSelectedTab =
		                Tab.fromInt(it.getInt(R.styleable.KinEcosystemTabsView_defaultSelected,
		                                      Tab.LEFT.ordinal))
            }
        } finally {
            attributes?.recycle()
        ...
        findViewById<LinearLayout>(R.id.tabs_bg).apply {
            background.setColorFilter(ContextCompat.getColor(context,
                                                             R.color.kinecosystem_tab_deselected_background_light), PorterDuff.Mode.SRC_ATOP)
        }


        textSelectedColor = ContextCompat.getColor(context,
                                                                                              R.color.kinecosystem_white)
        textDeselectedColor = ContextCompat.getColor(context,
                                                                                                R.color.kinecosystem_tab_deselected_text)
    ...
    private fun setSelectedTab(selectedTabIndex: Tab, animate: Boolean = true) {
        ...
        if (animate) {
            ...
            if (selectedTabIndex == Tab.LEFT) {
                ...
                val propertyXPos = PropertyValuesHolder.ofFloat(PROP_NAME_X_POS,
                                                                rightXPos,
                                                                leftXPos)
                val propertyColor = PropertyValuesHolder.ofObject(PROP_NAME_COLOR,
                                                                  argbEvaluator,
                                                                  rightColor,
                                                                  leftColor)
                ...
                ObjectAnimator.ofInt(leftTab,
                                     PROP_NAME_TEXT_COLOR, leftTab.currentTextColor,
                                     textSelectedColor).apply {
                    duration = DURATION_FADE_ANIM
                    setEvaluator(argbEvaluator)
                    start()
                }

                ObjectAnimator.ofInt(rightTab,
                                     PROP_NAME_TEXT_COLOR, rightTab.currentTextColor,
                                     textDeselectedColor).apply {
                    duration = DURATION_FADE_ANIM
                    setEvaluator(argbEvaluator)
                    start()
                }
